...
func (*ArticlesController) Update(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if !policies.CanModifyArticle(_article) {
			flash.Warning("未授权操作！")
			http.Redirect(w, r, route.Name2URL("articles.index"), http.StatusForbidden)
		} else {
			// 4.1 表单验证
			...
			if len(errors) == 0 {
				...
				// √ 更新成功，跳转到文章详情页
				if rowsAffected > 0 {
					http.Redirect(w, r, route.Name2URL("articles.show", "id", id), http.StatusFound)
				} else {
